...
async function persistConversationEntry(entry) {
  if (!SUPABASE_ENABLED) return;
  try {
    await supabaseRequest('/rest/v1/conversations',{method:'POST',headers:{Prefer:'return=minimal'},
...
אם המתקשר מבקש במפורש מידע עדכני מהאינטרנט, חדשות, מחיר/זמינות נוכחיים או "חיפוש באינטרנט", החזר בשורה הראשונה בדיוק SEARCH_REQUEST ולאחריה ניסוח קצר של השאלה.
אחרת אל תחזיר SEARCH_REQUEST.
...
async function callHandler(call) {
  const callerPhone=getCallerNumber(call),callId=call?.callId||call?.values?.ApiCallId||'';
...
async function configureYemotStructure() {
  ...
  console.log('Configuring Yemot API extension 1...'); await updateExtension('ivr2:/1',{type:'api',api_link:publicUrl+'/yemot'});
